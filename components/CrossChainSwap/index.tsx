...
const supportedTokens: Record<number, Token[]> = {
    10: [ // Optimism
        ...
        { symbol: "USDC.e", name: "USD Coin", address: "0x7F5c764cBc14f9669B88837ca1490cCa17c31607" },
        { symbol: "WETH", name: "Wrapped Ether", address: "0x4200000000000000000000000000000000000006" },
        { symbol: "WBTC", name: "Wrapped Bitcoin", address: "0x68f180fcCe6836688e9084f035309E29Bf0A2095" },
    ],
    // 可以添加其他鏈的代幣
};
    ...
    const executeSwap = async () => {
        ...
        try {
            ...
            if (isDevMode) {
                ...
                // 模擬成功響應
                alert(`模擬跨鏈兌換: ${amount} ${sourceToken.symbol} (${sourceChain.name}) → ${targetChain.name}`);
                return;
            }
            ...
            try {
                ...
                tokenAmount = tokenToDecimals(parseFloat(amount), sourceToken.symbol as Tokens).toString();
            } catch (error) {
                // 如果失敗，手動處理不同代幣的小數位數
                const decimals = {
                    "USDC.e": 6,
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        ...
                        <select
                            ...
                            onChange={(e) => {
                                const token = supportedTokens[sourceChain.id].find(t => t.symbol === e.target.value);
                    ...
                    <div>
                        ...
                        >
                            {supportedChains.filter(chain => chain.id !== sourceChain.id).map((chain) => (
                                <option key={chain.id} value={chain.id}>
                                    {chain.name}
                                </option>
                            ))}
